...
public final class CaptureStore extends SQLiteOpenHelper {
    ...
    @Override public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion != newVersion) throw new IllegalStateException("Missing capture migration " + oldVersion + " -> " + newVersion);
    }
    ...
    public Session getSession(String id) {
        ...
        Cursor c = getReadableDatabase().rawQuery("SELECT id,label,status,material_code,ot,shell_length_mm," +
                "accepted,rejected,low_mask,high_mask FROM session WHERE id=?", new String[]{id});
        try { return c.moveToFirst() ? readSession(c) : null; } finally { c.close(); }
    }

    public Session latestOpen() {
        Cursor c = getReadableDatabase().rawQuery("SELECT id,label,status,material_code,ot,shell_length_mm," +
                "accepted,rejected,low_mask,high_mask FROM session WHERE status='CAPTURING' ORDER BY updated_at DESC LIMIT 1", null);
        try { return c.moveToFirst() ? readSession(c) : null; } finally { c.close(); }
    }

    public List<Session> recent(int limit) {
        Cursor c = getReadableDatabase().rawQuery("SELECT id,label,status,material_code,ot,shell_length_mm," +
                "accepted,rejected,low_mask,high_mask FROM session ORDER BY updated_at DESC LIMIT ?",
    ...
    public void saveFrame(String sessionId, int seq, File file, double yaw, double pitch, double roll,
                          ...
                          String band, int sector, String sha256) {
    ...
    private static Session querySession(SQLiteDatabase db, String id) {
        Cursor c = db.rawQuery("SELECT id,label,status,material_code,ot,shell_length_mm," +
                "accepted,rejected,low_mask,high_mask FROM session WHERE id=?", new String[]{id});
        try { return c.moveToFirst() ? readSession(c) : null; } finally { c.close(); }
    }
