...
/** UI for multiscale ZIP diagnostics followed by primary-inlier track persistence. */
public final class ZipReprocessActivity extends Activity {
    ...
    private void buildUi() {
        ...
        TextView title = text("REPROCESAR ZIP Y FORMAR TRACKS · ALPHA48", 25, true);
        title.setTextColor(Color.rgb(18, 52, 73));
        root.addView(title);

        TextView description = text(
                "Primero ejecuta la comparación alpha46/alpha47 con pirámide multiescala y matriz fundamental. Después repite una pasada acotada para persistir tracks de tres o más vistas usando exclusivamente inliers de pares STRONG/USABLE. Las aristas BRIDGE se excluyen y nunca habilitan BA, nube métrica, CAD ni liberación industrial.",
                14, false);
        ...
        saveJsonButton = button("GUARDAR TRACKS / DIAGNÓSTICO JSON");
    ...
    private void startProcessing(Uri uri) {
        ...
        saveJsonButton.setEnabled(false);
        savePackageButton.setEnabled(false);
        status.setText("Abriendo ZIP y ejecutando comparación alpha46/alpha47…");
        status.setTextColor(Color.rgb(35, 84, 117));
        new Thread(() -> {
            MultiScaleZipReprocessor.Result completed = null;
            try {
                completed = MultiScaleZipReprocessor.process(
                        ZipReprocessActivity.this, uri,
                        message -> runOnUiThread(() -> status.setText(message)));
                final MultiScaleZipReprocessor.Result alpha47 = completed;
                multiscaleResult = alpha47;
                runOnUiThread(() -> status.setText(alpha47.summary
                        + "\n\nAlpha47 completo. Iniciando ensamblaje de tracks alpha48…"));

                final ImportedTrackZipAnalyzer.Result tracks =
                        ImportedTrackZipAnalyzer.process(
                                ZipReprocessActivity.this, uri, alpha47,
                                message -> runOnUiThread(() -> status.setText(message)));
                runOnUiThread(() -> showCompleted(alpha47, tracks));
            } catch (Exception error) {
                final MultiScaleZipReprocessor.Result available = completed;
                runOnUiThread(() -> showFailure(available, error));
            }
        }, "ImportedTrackZipReprocessor").start();
    }

    private void showCompleted(MultiScaleZipReprocessor.Result alpha47,
        ...
        status.setText(alpha47.summary + "\n\n" + tracks.summary);
    ...
    private void showFailure(MultiScaleZipReprocessor.Result available, Exception error) {
        processing = false;
        multiscaleResult = available;
        trackResult = null;
        selectButton.setEnabled(true);
        saveJsonButton.setEnabled(available != null);
        savePackageButton.setEnabled(available != null);
        ...
        if (available == null) {
            ...
            status.setText(available.summary
                    + "\n\nTRACKS ALPHA48 BLOQUEADOS\n" + message
                    + "\nPuede guardar el diagnóstico alpha47 disponible.");
            status.setTextColor(Color.rgb(145, 82, 0));
        }
    }
