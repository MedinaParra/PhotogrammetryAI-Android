...
public final class CadAssemblyPackageExporter {
    ...
    public static File build(Context context, CadAssemblyStore store, String assemblyId,
        ...
        try(ZipOutputStream zip=new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(output)))){
            ...
            for(CadAssemblyStore.Component component:components){
                if(component.sourceKind==CadAssemblyStore.SourceKind.STEP&&component.sourcePath!=null){
                    File source=new File(component.sourcePath);
                    if(source.isFile())putFile(zip,"step/"+safeFile(component.id+"_"+source.getName()),source);
                }
                CadMeshCache.Record mesh=meshes.get(component.id);
                if(mesh!=null&&mesh.ready()&&mesh.filePath!=null){
                    File meshFile=new File(mesh.filePath);
                    if(meshFile.isFile())putFile(zip,"mesh/"+safeFile(component.id+".skmesh"),meshFile);
                }
            }
    ...
    private static String manifest(String assemblyId,String sessionId,CaptureStore.Session session,
        ...
        for(int i=0;i<constraints.issues.size();i++){
            AssemblyConstraintCore.Issue issue=constraints.issues.get(i);
            if(i>0)json.append(',');
            json.append("{\"severity\":").append(q(issue.severity.name()))
                    .append(",\"code\":").append(q(issue.code))
                    .append(",\"partId\":").append(q(issue.partId))
        ...
        for(int i=0;i<components.size();i++){
            ...
            json.append("    {\"id\":").append(q(c.id))
                    ...
                    .append(",\"sourceKind\":").append(q(c.sourceKind.name()))
                    .append(",\"sourceSha256\":").append(q(c.sourceSha256))
                    .append(",\"kernelStatus\":").append(q(c.kernelStatus))
                    .append(",\"dimensionsMm\":{\"length\":").append(f(c.lengthMm))
                    .append(",\"diameter\":").append(f(c.diameterMm))
                    .append(",\"width\":").append(f(c.widthMm))
                    .append(",\"height\":").append(f(c.heightMm))
                    .append(",\"depth\":").append(f(c.depthMm))
                    .append(",\"bore\":").append(f(c.boreMm)).append("}")
                    .append(",\"transform\":{\"translationMm\":[")
                    .append(f(c.txMm)).append(',').append(f(c.tyMm)).append(',').append(f(c.tzMm))
                    .append("],\"rotationDeg\":[")
                    .append(f(c.rxDeg)).append(',').append(f(c.ryDeg)).append(',').append(f(c.rzDeg)).append("]}")
                    .append(",\"mesh\":");
            if(mesh==null){
                json.append("null");
            }else{
                json.append("{\"status\":").append(q(mesh.status))
                        .append(",\"sourceSha256\":").append(q(mesh.sourceSha256))
                        .append(",\"vertexCount\":").append(mesh.vertexCount)
                        .append(",\"triangleCount\":").append(mesh.triangleCount)
                        .append(",\"diagnostic\":").append(q(mesh.diagnostic))
                        .append(",\"archivePath\":")
            ...
            if(i+1<components.size())json.append(',');
            json.append('\n');
        }
        json.append("  ]\n}\n");
    ...
    private static void putFile(ZipOutputStream zip,String name,File file)throws Exception{
        ZipEntry entry=new ZipEntry(name);entry.setTime(0);zip.putNextEntry(entry);
        byte[] buffer=new byte[65536];
        try(BufferedInputStream input=new BufferedInputStream(new FileInputStream(file))){
            int count;while((count=input.read(buffer))>=0)if(count>0)zip.write(buffer,0,count);
        }
        zip.closeEntry();
    }
