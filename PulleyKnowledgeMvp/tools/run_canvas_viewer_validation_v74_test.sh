...
grep -q 'versionCode 58' "$GRADLE"
grep -q "versionName '0.18.0-alpha58'" "$GRADLE"
grep -q 'SKM Polea AI Lab2 alpha58' "$MANIFEST"
...
if grep -Eq 'GLSurfaceView|android\.opengl|GLES20|EGLConfig|GL10|shader' "$VIEWER"; then
  echo 'ERROR: alpha58 PointCloudViewerActivity must remain independent from GLSurfaceView/OpenGL/EGL/shaders' >&2
  exit 1
fi

grep -q 'Test Canvas viewer diagnostics and state restoration' "$WORKFLOW"
grep -q 'SKM-Polea-AI-Lab2-CAD-STEP-v0.18.0-alpha58-clean.zip' "$WORKFLOW"
...
if missing:
    raise SystemExit("Missing alpha58 Canvas diagnostics: " + ", ".join(missing))
if source.count("postInvalidateOnAnimation") < 5:
    raise SystemExit("Expected foreground, size and gesture redraw hooks")
print("alpha58 Canvas viewer diagnostics/state contract PASS")
PY
